...
export default function BestStudent() {
  const {
    data: bestStudentData,
    isLoading,
    isError,
    refetch,
  } = useGetWebsiteSectionQuery("bestStudent");
      ...
      >
        <div className="text-center py-5">
          <div className="alert alert-danger">
            Failed to load best student data.
            <button
  ...
  // Handle empty data
  if (!bestStudentData || !bestStudentData.name) {
    return (
      <div
        className="news-right-items-feb22 mb-0"
        style={{ minHeight: "300px" }}
      >
        <div className="text-center py-5">
          <div className="alert alert-info">
            No best student selected yet. Please select a best student from the
            dashboard.
          </div>
        </div>
      </div>
  ...
  const studentImage = photo || three;

  // Format title with achievements if available
  const title =
    "Best Student of the Month" + (achievements ? ` - ${achievements}` : "");
      ...
      >
        <div className="news-thumb col-lg-6 col-sm-6 border-2 px-3">
          <img
            ...
            alt={name || "Best Student"}
            data-aos-duration="800"
            data-aos="fade-up"
            data-aos-delay="300"
            onError={(e) => {
              e.target.onerror = null; // Prevent infinite loop
              e.target.src = three; // Fallback to default image
            }}
          />
        </div>
        <div className="news-content col-lg-6 col-sm-6">
          ...
          <p className={"paragraph_span pt-3"} style={{ cursor: "default" }}>
            {description || "No description available for the best student."}
          </p>
        </div>
      </div>
    </div>
