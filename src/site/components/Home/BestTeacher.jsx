...
import two from "../../assets/img/home/t.png";
...
export default function BestTeacher() {
      ...
      >
        <div className="text-center py-5">
          <div className="alert alert-danger">
            Failed to load best teacher data.
            <button
  ...
  // Handle empty data
  if (!bestTeacherData || !bestTeacherData.name) {
    return (
      <div
        className="news-right-items-feb22 mb-0"
        style={{ minHeight: "300px" }}
      >
        <div className="text-center py-5">
          <div className="alert alert-info">
            No best teacher selected yet. Please select a best teacher from the
            dashboard.
          </div>
        </div>
      </div>
  ...
  const { name, designation, delegation, description, photo } = bestTeacherData;

  // Use the photo from API or default image
  const teacherImage = photo || two;

  // Format title with delegation if available
  const title =
    "Best Teacher of the Month" + (delegation ? ` - ${delegation}` : "");
  ...
  return (
    ...
    >
      ...
      <div
        className="row border-2 border border-gray rounded-4 py-3"
        style={{}}
      >
        <div className="news-thumb col-lg-6 col-sm-6 border-2 px-3">
          <img
            ...
            alt={name || "Best Teacher"}
            data-aos-duration="800"
            data-aos="fade-up"
            data-aos-delay="300"
            onError={(e) => {
              e.target.onerror = null; // Prevent infinite loop
              e.target.src = two; // Fallback to default image
            }}
          />
        </div>
        <div className="news-content col-lg-6 col-sm-6">
          ...
          <p className={"paragraph_span pt-3"} style={{ cursor: "default" }}>
            {description || "No description available for the best teacher."}
          </p>
        </div>
      </div>
    </div>
