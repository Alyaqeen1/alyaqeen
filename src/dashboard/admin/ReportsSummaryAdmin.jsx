...
export default function ReportsSummaryAdmin() {
  ...
  const isLoadingInitial = showOverallSummary
    ? isLoadingYearly
    : isLoadingMonthly;
  const isLoading = isLoadingInitial || isFetching;
  ...
  useEffect(() => {
    if (!showOverallSummary && month) {
      refetchMonthly();
    }
  }, [month, year, showOverallSummary, refetchMonthly]);

  useEffect(() => {
    if (showOverallSummary) {
      refetchYearly();
    }
  }, [year, showOverallSummary, refetchYearly]);
