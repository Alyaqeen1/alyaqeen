...
export default function ReportsSummary() {
  ...
  const teacherReports = allReports.filter(
    (report) => report.teacher_id === teacher?._id,
  );

  // Filter by academic year
  const filteredByYear = teacherReports.filter(
    (report) => report.academic_year === academicYear,
  );

  // Filter by view mode
  const filteredReports = filteredByYear.filter((report) => {
    if (viewMode === "all") return true;
    if (viewMode === "beginning")
      return report.report_type === "beginning_of_year";
    if (viewMode === "ending") return report.report_type === "end_of_year";
    return true;
  });

  // Search filter
  const displayData = filteredReports.filter((report) => {
  ...
  const groupedReports = displayData.reduce((acc, report) => {
  ...
  const groupedData = Object.values(groupedReports);

  // Handle publish (make report visible to parents)
  ...
  // ===== FIXED: Get all progress fields with full details =====
  ...
  const getReportTypeLabel = (reportType) => {
    if (reportType === "beginning_of_year") return "📘 Beginning of Year";
    if (reportType === "end_of_year") return "📗 End of Year";
    return reportType;
  };

  const isLoading = isLoadingTeacher || isLoadingReports || isFetchingReports;
      ...
      {!isLoading && !isError && displayData.length > 0 && (
        <div className="mb-3">
          <small className="text-muted">
            Showing {groupedData.length} student
      ...
      {isLoading ? (
        ...
        <div className="row g-3">
          {groupedData.map((group, idx) => {
            const isGiftMuslim = group.type === "gift_muslim";
              ...
              <div key={idx} className="col-lg-6 col-xl-4">
                <div className="card shadow-sm border-0 h-100">
                  <div className="card-body d-flex flex-column">
                    ...
                    {hasEnding && (
                      <div className="mb-2 p-2 bg-light rounded">
                        <h6 className="mb-1 small text-warning">📗 End</h6>
                        {getProgressInfo(group.ending.lessons, group.type).map(
                          (info, i) => (
                            <div
                              key={i}
                              className="d-flex justify-content-between small"
